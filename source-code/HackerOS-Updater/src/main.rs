        ...
        Self {
            ...
            tasks: vec![
                ...
                Task {
                    ...
                    // POPRAWA: escapowanie cudzysłowów w stringu
                    command: "zsh -c \"source ~/.zshrc && omz update\"".to_string(),
                    is_sudo: false,
                    status: TaskStatus::Pending,
                },
                Task {
    ...
    fn start_updates(&mut self, tx: mpsc::Sender<AppEvent>, password: String) {
        ...
        tokio::spawn(async move {
            for (idx, task) in tasks.iter().enumerate() {
                let _ = tx
                    .send(AppEvent::TaskStatusChange(idx, TaskStatus::Running))
                    .await;
                let _ = tx
                    .send(AppEvent::LogLine(format!(">>> STARTED: {}", task.name)))
                    .await;
                ...
                match cmd_builder.spawn() {
                    Ok(mut child) => {
                            ...
                            for line in reader.lines() {
                                if let Ok(l) = line {
                                    if !l.contains("[sudo] password") {
                                        let _ = tx_clone_err
                                            .blocking_send(AppEvent::LogLine(format!("STDERR: {}", l)));
                                    }
                                }
                            }
                        });
                        ...
                        match status {
                            Ok(s) if s.success() => {
                                let _ = tx
                                    .send(AppEvent::TaskStatusChange(idx, TaskStatus::Success))
                                    .await;
                            }
                            _ => {
                                let _ = tx
                                    .send(AppEvent::TaskStatusChange(idx, TaskStatus::Failed))
                                    .await;
                            }
                        }
                    }
                    Err(e) => {
                        let _ = tx
                            .send(AppEvent::LogLine(format!("EXEC ERROR: {}", e)))
                            .await;
                        let _ = tx
                            .send(AppEvent::TaskStatusChange(idx, TaskStatus::Failed))
                            .await;
                ...
                let _ = tx
                    .send(AppEvent::LogLine(format!(">>> FINISHED: {}\n", task.name)))
                    .await;
            }
        ...
        if crossterm::event::poll(Duration::from_millis(50))? {
            if let Event::Key(key) = event::read()? {
                ...
                match app.state {
                    ...
                    AppState::Processing | AppState::Finished => match key.code {
                        KeyCode::Up => {
...
fn render_login(f: &mut Frame, area: Rect, app: &App) {
    ...
    let inner_layout = Layout::default()
        .direction(Direction::Vertical)
        .constraints([Constraint::Length(4), Constraint::Length(3), Constraint::Min(1)])
        .margin(2)
        .split(popup_area);

    let text = Paragraph::new(
        "Root privileges are required to perform system updates.\nPlease enter your sudo password below.",
    )
...
fn render_dashboard(f: &mut Frame, area: Rect, app: &App) {
    ...
    let completed_count = app
        .tasks
        .iter()
        .filter(|t| t.status == TaskStatus::Success || t.status == TaskStatus::Failed)
        .count();
