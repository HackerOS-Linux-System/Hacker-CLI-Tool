...
pub fn run_command_with_spinner(program: &str, args: Vec<&str>, message: &str) {
    println!("{}", format!("{}: {}", message, args.join(" ")).blue().bold());
    ...
    let handle = thread::spawn(move || {
        ...
        while !stop_clone.load(Ordering::Relaxed) {
            print!("\r{}", spinner_chars[i].purple());
    ...
    if output.status.success() {
        ...
        println!("{}", String::from_utf8_lossy(&output.stderr).red().bold());
    }
}

pub fn handle_update() {
    println!("{}", "========== Starting System Update ==========".magenta().bold());
    ...
    println!("{}", "========== System Update Complete ==========".green().bold());
}
